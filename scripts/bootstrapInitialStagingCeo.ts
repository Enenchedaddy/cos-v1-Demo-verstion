/** Staging-only, one-time bootstrap for the first CEO after the technical admin is active. */
...
const email = required('COS_BOOTSTRAP_CEO_EMAIL').toLowerCase();
const firstName = required('COS_BOOTSTRAP_CEO_FIRST_NAME');
const lastName = required('COS_BOOTSTRAP_CEO_LAST_NAME');
const jobTitle = required('COS_BOOTSTRAP_CEO_JOB_TITLE');
const department = required('COS_BOOTSTRAP_CEO_DEPARTMENT');
...
if (inviteError || !invitation.user) throw new Error('The CEO invitation could not be created. Do not retry blindly.');
const requestId = crypto.randomUUID();
const { error: bootstrapError } = await service.rpc('bootstrap_initial_ceo', {
...
if (bootstrapError) throw new Error('Invitation exists but CEO profile bootstrap failed. Do not retry blindly; correlation ' + requestId + '.');
console.log('Initial CEO invitation created. Correlation: ' + requestId);
