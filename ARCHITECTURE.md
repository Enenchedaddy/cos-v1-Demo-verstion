...
COS is a Vite + React + TypeScript single-page application. There is no active Express server, controller layer, or application-owned API runtime in this repository. Supabase is the persistence and authentication boundary when it is configured; otherwise the application runs against typed in-memory demo data.

```
main.tsx
  -> App.tsx (application shell and workspace selection)
    -> app/usePortalData.ts (portal data state, Supabase sync, optimistic writes)
...
1. `usePortalData` initializes state from `src/data.ts`.
2. If `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` are configured, it loads the same collections from Supabase.
3. Workspace components receive data and typed update callbacks through props.
4. An update first changes local state, then writes the collection through the same Supabase boundary. Failed writes retain the existing local demo behaviour and log a safe diagnostic.
...
- Preserve existing local-demo fallback when changing persistence code.
