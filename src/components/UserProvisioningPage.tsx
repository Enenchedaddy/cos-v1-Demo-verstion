...
import { isCeoApprovalAllowed, isInvitationAllowed } from '../auth/provisioningPolicy';
import {
  PROVISIONING_ROLE_CODES,
  invokeProvisioning,
...
const EMPTY = {
  ...
  jobTitle: '',
  department: '',
  roleCode: 'SALES' as ProvisioningRoleCode,
};
...
export default function UserProvisioningPage() {
  ...
  const perform = async (action: ReviewAction, request: ProvisioningRequest) => {
    setBusy(`${action}:${request.id}`);
    setError(null);
    setMessage(null);
    try {
      const input: Record<string, unknown> = { requestId: request.id };
      if (action === 'reject_ceo') {
        input.reason = 'Rejected through controlled provisioning review.';
      }
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setForm(EMPTY);
      setMessage('Provisioning request created and awaiting CEO approval.');
      await refresh();
    } catch (nextError) {
    ...
    <main className="min-h-screen bg-[#F5F7FA] px-4 py-6 text-[#15202B] sm:px-7 lg:px-10">
      <div className="mx-auto max-w-7xl">
        ...
        <header className="mt-5 flex flex-wrap items-end justify-between gap-4 border-b border-[#D8D6CE] pb-5">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#4065B3]">Controlled identity operations</p>
            <h1 className="mt-1 text-3xl font-semibold tracking-[-0.03em]">User provisioning</h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-[#5E6872]">CEO approval makes a request ready for invitation. The technical administrator sends the secure invitation using the approved request data.</p>
          </div>
          <button type="button" onClick={() => void refresh()} disabled={busy === 'refresh'} className="inline-flex min-h-11 items-center gap-2 border border-[#183153] px-4 text-sm font-semibold text-[#183153] disabled:opacity-50">
        ...
        {canRequest && <section className="mt-6 border border-[#D8D6CE] bg-white p-5 sm:p-6">
          ...
          <form className="mt-5 grid gap-4 sm:grid-cols-2" onSubmit={submit}>
            {(['firstName', 'lastName', 'email', 'jobTitle', 'department'] as const).map((field) => <label key={field} className="block">
              <span className="mb-2 block text-xs font-semibold capitalize">{field.replace(/([A-Z])/g, ' $1')}</span>
              <input required={field === 'firstName' || field === 'lastName' || field === 'email'} type={field === 'email' ? 'email' : 'text'} value={form[field]} onChange={(event) => setForm({ ...form, [field]: event.target.value })} className="min-h-11 w-full border border-[#D8D6CE] bg-[#FCFBF7] px-3 text-sm" />
            </label>)}
            <label className="block">
              <span className="mb-2 block text-xs font-semibold">Approved global role</span>
              <select value={form.roleCode} onChange={(event) => setForm({ ...form, roleCode: event.target.value as ProvisioningRoleCode })} className="min-h-11 w-full border border-[#D8D6CE] bg-[#FCFBF7] px-3 text-sm">
                {PROVISIONING_ROLE_CODES.map((code) => <option key={code} value={code}>{ROLE_LABELS[code]}</option>)}
              </select>
            </label>
            <div className="sm:col-span-2"><button disabled={busy === 'create'} className="inline-flex min-h-11 items-center gap-2 bg-[#183153] px-4 text-sm font-semibold text-white disabled:opacity-50"><Send size={16} /> Create request</button></div>
          </form>
        </section>}

        <section className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ['Pending requests', grouped.pending.length],
...
function Action({ label, action, request, busy, onAction }: { label: string; action: ReviewAction; request: ProvisioningRequest; busy: string | null; onAction: (action: ReviewAction, request: ProvisioningRequest) => void }) {
