...
function Header({ title, onClose, onExit, showLogo = true }: { title: string; onClose?: () => void; onExit?: () => void; showLogo?: boolean }) {
  ...
  return <header className="sm-sidebar__header">{showLogo && <COSLogo className="h-8 w-8 shrink-0" variant="white" />}<div className="min-w-0 flex-1"><p>Central Operating System</p><h2 title={title}>{title}</h2></div>{(onClose || onExit) && <button type="button" onClick={handleHeaderAction} aria-label="Close Management sidebar"><X size={20} /></button>}</header>;
}
