...
function Header({ title, onClose }: { title: string; onClose?: () => void }) {
  return <header className="sm-sidebar__header"><COSLogo className="h-8 w-8 shrink-0" variant="white" /><div className="min-w-0 flex-1"><p>Central Operating System</p><h2 title={title}>{title}</h2></div>{onClose && <button type="button" onClick={onClose} aria-label="Close Management sidebar"><X size={20} /></button>}</header>;
}
...
export default function ManagementSidebar({ activeArea, activeChildId, mode, onAreaSelect, onChildSelect, onBackToMain, onClose }: ManagementSidebarProps) {
